...
def get_m_stats_matrix(data_set_name, study_output_filepath, ncbi_wrapper: NCBITaxonomistWrapper, overlap_manager: OverlapManager, cross_hit_threshold: float = 0.3, min_taxonomic_score: float = 0.7, best_match_level: str = NCBITaxonomistWrapper.TAX_SPECIES, filter_no_leaf = True, log = False):
    """
    ...
    m_stats = _compute_best_matches(m_stats, input_taxids, ncbi_wrapper,
                                    overlap_manager, min_taxonomic_score)
    if log:
...
def _get_m_stats_paths(data_set_name, study_output_filepath):
    ...
    return {
        'matched_assemblies': os.path.join(
            study_output_filepath, data_set_name, "output", "matched_assemblies.tsv"),
        'merged_stats': os.path.join(
            study_output_filepath, data_set_name, "output", "merged_coverage_statistics.tsv"),
        'input': os.path.join(
            study_output_filepath, data_set_name, "input", f"{data_set_name}.tsv"),
        'classification': os.path.join(
            study_output_filepath, data_set_name, "classification",
            f"{data_set_name}_merged_classification.tsv"),
    }
...
def _load_m_stats_inputs(paths):
    ...
    m_stats = pd.read_csv(paths['merged_stats'], sep="\t").rename(
        columns={"#rname": "assembly_accession"})
    if "error_rate" in m_stats.columns:
        m_stats = m_stats[['assembly_accession', "coverage", "covbases",
                           "meanmapq", "numreads", "error_rate", "file"]]
    else:
        m_stats = m_stats[['assembly_accession', "coverage", "covbases",
                           "meanmapq", "numreads", "file"]]
...
def _resolve_assembly_classifications(matched, classification):
    ...
    matched = matched.sort_values(by=['total_uniq_reads'], ascending=False)\
...
def _compute_best_matches(m_stats, input_taxids, ncbi_wrapper, overlap_manager,
                          min_taxonomic_score):
    ...
    m_stats = m_stats.apply(
        lambda row: update_df_best_match(
            row, input_taxids, ncbi_wrapper,
            min_score=min_taxonomic_score,
            best_level=NCBITaxonomistWrapper.TAX_SPECIES),
        axis=1)
    ...
    for _, group in m_stats.groupby('best_match_taxid'):
        group = group.sort_values(
            by=['best_match_score', 'coverage', 'error_rate'],
            ascending=[False, False, True]).reset_index(drop=True)
        found = False
        for ix, row in group.iterrows():
            if row['coverage'] == 0.0:
                continue
            if ncbi_tools.level_is_below(row['best_match_level'],
                                          NCBITaxonomistWrapper.TAX_SPECIES):
                continue
            if not found:
...
def node_total_true_leaves(overlap_manager, node, m_stats_stats_matrix) -> list:
    ...
    leaf_taxids = m_stats_stats_matrix[(m_stats_stats_matrix.index.isin(leaves)) & (m_stats_stats_matrix['best_match_is_best'] == True)]['best_match_taxid'].dropna().tolist()
...
def get_subset_composition_counts(node_data, tax_data: pd.DataFrame, tax_level: str = "order", count_column: str = "total_uniq_reads"):
    ...
    counts_by_taxa = pd.concat([counts_by_taxa, pd.DataFrame({tax_level: missing_orders, 'total_uniq_reads': [0]*len(missing_orders), 'proportion': [0.0]*len(missing_orders)})], ignore_index=True)
...
def node_composition_with_stats(node_data, tax_data: pd.DataFrame, tax_level: str = "order", count_column: str = "total_uniq_reads"):
    ...
    tax_diversity_shannon = shannon_diversity_from_list(
        node_data[tax_level].dropna().tolist()
    )
...
def compute_node_purity(overlap_manager, m_stats_stats_matrix) -> pd.DataFrame:
    ...
    m_stats_stats_matrix['leaf'] = m_stats_stats_matrix.apply(lambda row: overlap_manager.get_leaf_from_assid(row['assid']), axis=1)
