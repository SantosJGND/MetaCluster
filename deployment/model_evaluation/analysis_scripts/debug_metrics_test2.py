...
print(
    f"  Expected recall (TP/|input|):      {expected_recall:.4f}  (matches new: {abs(expected_recall - new_rec) < 1e-10})"
)
print(
    f"  Expected precision (TP/|output|):  {expected_prec:.4f}   (matches new: {abs(expected_prec - new_prec) < 1e-10})"
)
...
if models is not None:
    ...
    try:
        ...
        new_m_stats = get_m_stats_matrix(
            DATASET_NAME, str(config.study_output_filepath), ncbi_wrapper, filtered_om, filter_no_leaf=False
        )
...
m_stats_leaf = get_m_stats_matrix(
    DATASET_NAME, str(config.study_output_filepath), ncbi_wrapper, overlap_manager, filter_no_leaf=True
)
...
vals = [clade_prec if results_df is not None else 0.0, clade_prec_fixed, clade_rec if results_df is not None else 0.0]
...
all_datasets = sorted(
    [
        d
        for d in os.listdir(config.study_output_filepath)
...
for ds in ["dataset_0071_plan"]:
    try:
        ...
        if om is None:
            continue
        m_stats = get_m_stats_matrix(ds, str(config.study_output_filepath), ncbi_wrapper, om, filter_no_leaf=True)
        ...
        try:
            full_df = predict_data_set_clades_composition(
                ds, m_stats, om, models.composition_modeller, taxids_to_use, tax_level=config.tax_level
            )
            ...
            if prec_full == 0.0:
                ...
                print("##################3")
                m_stats = get_m_stats_matrix(
                    ds, str(config.study_output_filepath), ncbi_wrapper, om, filter_no_leaf=False, log=True
                )
                full_df = predict_data_set_clades_composition(
                    ds, m_stats, om, models.composition_modeller, taxids_to_use, tax_level=config.tax_level
                )
        ...
        try:
            ...
            post_df = predict_data_set_clades_composition(
                ds, m_stats, om2, models.composition_modeller, taxids_to_use, tax_level=config.tax_level
            )
        ...
        try:
            ...
            fixed_df = predict_data_set_clades_fixed(ds, m_stats, om2, min_dist_threshold=0.6)
        ...
        records.append(
            {
                ...
                "rec_full": rec_full,
                "n_nodes_full": n_full,
                "n_pred_full": n_pred_full,
                "failure_full": failure_full,
                "prec_post": prec_post,
                "n_nodes_post": n_post,
                "n_pred_post": n_pred_post,
                "failure_post": failure_post,
                "prec_fixed": prec_fixed,
                "n_nodes_fixed": n_fixed,
                "n_pred_fixed": n_pred_fixed,
...
for method, fcol in [("full", "failure_full"), ("post", "failure_post"), ("fixed", "failure_fixed")]:
    ...
    for k in ["OK", "A", "B", "C"]:
        v = counts.get(k, 0)
        lbl = {"OK": ">0 precision", "A": "empty predicted set", "B": "no intersection", "C": "exception"}[k]
        print(f"    {lbl:20s}: {v}")
