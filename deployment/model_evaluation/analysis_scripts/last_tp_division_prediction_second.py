...
def get_args():
    ...
    parser.add_argument("--output_dir", type=str, default="last_tp_division_outputs",
...
KERNELS = {
    ...
    'RBF+RQ':           ConstantKernel(1.0) * (RBF(length_scale=1.0)
...
stats_features = ['counts_kurtosis', 'counts_skewness', 'tax_diversity_shannon',
...
training_data = training_data[
    (training_data['last_best_match_relindex'] > 0.0) &
    (training_data['last_best_match_relindex'] < 1.0)
]
...
def compute_ground_truth_for_tau(y_test_array, tau, n_test):
    ...
    return np.array([
        compute_ground_truth_division(y_test_array[:, s], tau)
...
def run_grid_search_for_tau(precomputed, deterministic_preds, ground_truth,
                             model_keys, n_test, tau,
                             X_vals, lik_tau_vals, asym_ratios):
    ...
    for mk in model_keys:
        ...
        for over_cost, under_cost in asym_ratios:
            for X_val in X_vals:
                ...
                rows.append({'model': mk, 'approach': 'CLF', 'param': X_val,
                             'over_cost': over_cost, 'under_cost': under_cost,
                             'loss': loss, 'tau': tau})
            for lik_tau in lik_tau_vals:
                yp = predict_division_lik(lik, lik_tau, DIVISION_FRACTIONS)
                loss = asymmetric_loss(ground_truth, yp, over_cost, under_cost)
                rows.append({'model': mk, 'approach': 'Likelihood', 'param': lik_tau,
                             'over_cost': over_cost, 'under_cost': under_cost,
                             'loss': loss, 'tau': tau})
    for mk, mu in deterministic_preds.items():
        yp = np.array([compute_ground_truth_division(mu[:, s], tau)
                       for s in range(n_test)])
        for over_cost, under_cost in asym_ratios:
            loss = asymmetric_loss(ground_truth, yp, over_cost, under_cost)
            rows.append({'model': mk, 'approach': 'Direct', 'param': 0.0,
                         'over_cost': over_cost, 'under_cost': under_cost,
                         'loss': loss, 'tau': tau})
...
def find_optimal_params(results_df):
    ...
    for keys, group in results_df.groupby(
            ['model', 'approach', 'over_cost', 'under_cost', 'tau']):
...
def evaluate_actual_recall_at_prediction(y_pred, y_test_array, n_test):
    ...
    return np.array([actual_recall_at_fraction(y_test_array[:, s], y_pred[s])
                     for s in range(n_test)])
...
for split_name, y_df, X_df, X_scaled in [
    ...
    gt_arr = np.array([compute_ground_truth_division(y_arr[s], BASELINE_TAU)
                       for s in range(len(y_arr))])
    valid = gt_arr < 1.0
    n_before = len(y_df)
    n_after = int(valid.sum())
    n_removed = n_before - n_after
    if n_removed > 0:
        print(f"  {split_name}: removed {n_removed}/{n_before} samples "
              f"(keep {n_after})")
...
for kernel_name, base_kernel in KERNELS.items():
    ...
    for col in y_columns_active:
        ...
        gp = GaussianProcessRegressor(
            kernel=deepcopy(kernel), normalize_y=True,
            n_restarts_optimizer=3, random_state=42
        )
...
X_gs_tr, X_gs_val, y_gs_tr, y_gs_val = train_test_split(
    X_train_scaled, y_train[y_columns_active], test_size=0.2, random_state=42,
)
...
for n_est, lr, mb in product([200, 500], [0.01, 0.05], [0.5, 1.0]):
    ...
    for d in rep_divs:
        ...
        model = NGBRegressor(
            ...
            minibatch_frac=mb, verbose=False, random_state=42,
        )
...
for d in trainable:
    col = y_columns_active[d]
    model = NGBRegressor(
        Dist=Normal, **best_params_ngb, verbose=False, random_state=42,
    )
...
ngb_preds = _pad_recall(np.array([
    ngb_models[col].predict(X_test_scaled) if ngb_models[col] is not None
    else np.full(len(X_test_scaled), 1.0)
    for col in y_columns_active
]))
...
gpclf_actual_recalls = np.array([
    actual_recall_at_fraction(y_test_array[:, s], gpclf_y_pred[s])
    for s in range(n_test)
])

# Diagnostics
print(f"  y_pred: min={gpclf_y_pred.min():.3f}, max={gpclf_y_pred.max():.3f}, "
      f"mean={gpclf_y_pred.mean():.3f}, std={gpclf_y_pred.std():.3f}")
raw = gpclf_model.predict_raw(X_test)
print(f"  Raw recall preds (mean \u00b1 std across test samples per division):")
for d in range(N_ACTIVE_DIVISIONS):
    print(f"    div {d:2d} ({DIVISION_FRACTIONS[d]:.2f}): "
          f"{raw[:, d].mean():.4f} \u00b1 {raw[:, d].std():.4f}")
...
for div_idx, label in [(0, 'first'), (-1, 'last')]:
    ...
    print(f"  P(recall>{tau_gpclf:.2f}) @ {label} div: "
          f"mean={p.mean():.3f} \u00b1 {p.std():.3f}")
print()
...
for tau in TARGET_TAUS:
    ...
    results_df = run_grid_search_for_tau(
        precomputed, deterministic_preds, ground_truth,
        model_keys, n_test, tau,
        X_VALUES, LIK_TAU_VALUES, ASYMMETRIC_RATIOS
    )
    ...
    print(f"  Actual recall at cutoff: mean={gpclf_actual_recalls.mean():.3f}, "
    ...
    for (m, a, oc, uc), g in summary_df.groupby(
            ['model', 'approach', 'over_cost', 'under_cost']):
        best = g.loc[g['loss'].idxmin()]
        print(f"  {m:12s} | {a:11s} | over:{oc} under:{uc} "
              f"| param={best['param']:.2f} | loss={best['loss']:.4f}")
        ...
        for mk in model_keys:
            ...
            for approach in ['CLF', 'Likelihood']:
                for oc, uc in [(1, 1), (3, 1), (10, 1)]:
                    ...
                    ax.plot(best['param'], best['loss'], 'o',
                            label=f'{approach} {oc}:{uc}')
    ...
    heat_data = summary_df.groupby(
        ['model', 'approach', 'over_cost', 'under_cost']
    )['loss'].min().reset_index()
    heat_pivot = heat_data.pivot_table(
        index='model', columns=['approach', 'over_cost', 'under_cost'],
        values='loss', aggfunc='min'
    )
    n_models = len(heat_pivot)
    fig_h = max(6, n_models * 0.45)
    fig, ax = plt.subplots(figsize=(16, fig_h))
    sns.heatmap(heat_pivot, annot=True, fmt='.4f', cmap='YlOrRd_r',
                ax=ax, cbar_kws={'label': 'Best loss'},
                linewidths=0.5, linecolor='gray')
    ax.set_title(f'Best asymmetric loss by model, approach, and ratio (\u03c4={tau})',
                 fontsize=12)
    ax.set_ylabel('')
    plt.tight_layout()
    ...
    if tau == BASELINE_TAU:
        ...
        for (m, a), g in summary_df.groupby(['model', 'approach']):
            ...
            ratio_data['ratio'] = ratio_data.apply(
                lambda r: f"{r['over_cost']}:{r['under_cost']}", axis=1)
            ax.plot(ratio_data['ratio'], ratio_data['loss'], 'o-', lw=1.5,
                    label=f'{m} \u2014 {a}')
    ...
    for mk in model_keys + list(deterministic_preds.keys()):
        for approach in ['CLF', 'Likelihood', 'Direct']:
            ...
            if approach == 'Direct':
                ...
                y_pred = np.array([
                    compute_ground_truth_division(mu[:, s], tau)
                    for s in range(n_test)
                ])
            else:
                prob = precomputed[mk]['prob']
            ...
            actual_recalls = evaluate_actual_recall_at_prediction(
                y_pred, y_test_array, n_test)
    ...
    gt_train = np.array([compute_ground_truth_division(y_train_array[:, s], tau)
                         for s in range(n_train)])

    for mk_direct in ['RF', 'XGBoost']:
        mu_train = _pad_recall((rf_model if mk_direct == 'RF' else xgb_model).predict(X_train_scaled).T)
        fracs_train = np.array([compute_ground_truth_division(mu_train[:, s], tau)
                                for s in range(n_train)])
        mu_test = deterministic_preds[mk_direct]
        fracs_test = np.array([compute_ground_truth_division(mu_test[:, s], tau)
                               for s in range(n_test)])
        ...
        print(f"  {mk_direct} + isotonic: loss={cal_loss:.4f}, "
    ...
    for frac_name, model_cls, frac_params in [
        ...
        print(f"  {frac_name}: loss={frac_loss:.4f}, "
              f"mean_recall={np.mean(frac_recalls):.3f}")
    ...
    print(f"  GP-DirectXGB: loss={gp_meta_loss:.4f}, "
          f"mean_recall={np.mean(gp_meta_recalls):.3f}")
    ...
    if tau == BASELINE_TAU:
        ...
        for mk in list(posterior.keys()) + list(deterministic_preds.keys()):
            for approach in ['CLF', 'Likelihood', 'Direct']:
                ...
                ax.scatter(ground_truth[over], y_pred[over], c='red', alpha=0.5, s=20,
                           label=f'Over ({over.sum()})')
                ax.scatter(ground_truth[under], y_pred[under], c='blue', alpha=0.5, s=20,
                           label=f'Under ({under.sum()})')
                ...
                ax.axvline(np.median(errors), color='red', ls='-', lw=1.5,
        ...
        ax.scatter(y_true_gpclf_sc[over], gpclf_y_pred[over], c='red', alpha=0.5, s=20,
                   label=f'Over ({over.sum()})')
        ax.scatter(y_true_gpclf_sc[under], gpclf_y_pred[under], c='blue', alpha=0.5, s=20,
                   label=f'Under ({under.sum()})')
        ax.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
        ax.set_xlabel('True fraction')
        ax.set_ylabel('Predicted fraction')
        ...
        for mk_direct in ['RF', 'XGBoost']:
            ...
            ax.scatter(ground_truth[over], fracs_cal[over], c='red', alpha=0.5, s=20,
                       label=f'Over ({over.sum()})')
            ax.scatter(ground_truth[under], fracs_cal[under], c='blue', alpha=0.5, s=20,
                       label=f'Under ({under.sum()})')
            ax.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
            ax.set_xlabel('True fraction')
            ax.set_ylabel('Predicted fraction')
        ...
        for frac_name, model_cls, frac_params in [
            ...
            ax.scatter(ground_truth[over], frac_preds[over], c='red', alpha=0.5, s=20,
                       label=f'Over ({over.sum()})')
            ax.scatter(ground_truth[under], frac_preds[under], c='blue', alpha=0.5, s=20,
                       label=f'Under ({under.sum()})')
            ax.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
            ax.set_xlabel('True fraction')
            ax.set_ylabel('Predicted fraction')
        ...
        ax.scatter(ground_truth[over], gp_meta_preds[over], c='red', alpha=0.5, s=20,
                   label=f'Over ({over.sum()})')
        ax.scatter(ground_truth[under], gp_meta_preds[under], c='blue', alpha=0.5, s=20,
                   label=f'Under ({under.sum()})')
        ax.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
        ax.set_xlabel('True fraction')
        ax.set_ylabel('Predicted fraction')
    ...
    if tau == BASELINE_TAU:
        ...
        ax.boxplot(bp_data, positions=range(1, N_DIVISIONS + 1),
        ...
        ax.plot(range(1, N_DIVISIONS + 1), means, 'ko-', lw=2, markersize=5,
                label='Mean recall')
        ...
        ax.set_xticklabels([f'{p}\n({DIVISION_FRACTIONS[p-1]:.0%})' for p in range(1, N_DIVISIONS + 1)],
                           fontsize=7)
        ...
        for mk in model_keys + list(deterministic_preds.keys()):
            for approach in ['CLF', 'Likelihood', 'Direct']:
                ...
                dist_summary_rows.append({
                    ...
                    'param_opt': param_opt, 'loss': best_loss,
                    'mean_actual_recall': np.mean(actual_recalls),
                    'median_actual_recall': np.median(actual_recalls),
                ...
                ax = axes[0]
                ax.hist(actual_recalls, bins=25, alpha=0.7, color='steelblue',
                        edgecolor='black', density=True)
                ax.axvline(tau, color='red', ls='--', lw=2, label=f'\u03c4 = {tau}')
                ax.axvline(np.mean(actual_recalls), color='darkorange', ls='-', lw=2,
                           label=f'Mean = {np.mean(actual_recalls):.3f}')
                ax.axvline(np.median(actual_recalls), color='green', ls=':', lw=2,
                           label=f'Median = {np.median(actual_recalls):.3f}')
                ax.set_xlabel('Actual recall at predicted last-index', fontsize=11)
                ax.set_ylabel('Density', fontsize=11)
                ax.set_title(f'{mk} \u2014 {approach}\n'
                             f'\u03c4={tau}, param={param_opt:.2f}, loss={best_loss:.4f}',
                             fontsize=10)
                ax.legend(fontsize=8)
                ax.grid(True, alpha=0.3, axis='y')
                ...
                for s in sample_idxs:
                    ax.plot(DIVISION_FRACTIONS, y_test_array[:, s],
                ...
                for s in range(n_test):
                    ...
                    ax.scatter(y_pred[s] + jitter[s], actual_recalls[s],
        ...
        dist_summary_rows.append({
            'model': 'GPCLFThreshold', 'approach': 'CLF',
            'param_opt': gpclf_model.x_thresh, 'loss': gpclf_loss_tau,
            'mean_actual_recall': np.mean(gpclf_actual_recalls),
            'median_actual_recall': np.median(gpclf_actual_recalls),
            'pct_below_tau': gpclf_below,
        })
        for mk_direct in ['RF', 'XGBoost']:
            ...
            cal_recalls = evaluate_actual_recall_at_prediction(fracs_cal, y_test_array, n_test)
            dist_summary_rows.append({
                'model': mk_direct, 'approach': 'Direct+ISO',
                'param_opt': round(float(fracs_cal.mean() - fracs_test.mean()), 4),
                'loss': cal_loss,
                'mean_actual_recall': np.mean(cal_recalls),
                'median_actual_recall': np.median(cal_recalls),
        ...
        for frac_name, model_cls, frac_params in [
            ...
            frac_recalls = evaluate_actual_recall_at_prediction(frac_preds, y_test_array, n_test)
            dist_summary_rows.append({
                'model': frac_name, 'approach': 'Direct',
                'param_opt': 0.0, 'loss': frac_loss,
                'mean_actual_recall': np.mean(frac_recalls),
                'median_actual_recall': np.median(frac_recalls),
                'pct_below_tau': (frac_recalls < tau).mean(),
            })
        dist_summary_rows.append({
            'model': 'GP-DirectXGB', 'approach': 'Direct',
            'param_opt': 0.0, 'loss': gp_meta_loss,
            'mean_actual_recall': np.mean(gp_meta_recalls),
            'median_actual_recall': np.median(gp_meta_recalls),
            'pct_below_tau': (gp_meta_recalls < tau).mean(),
        })
        ...
        ax.scatter(y_true_gpclf[over], gpclf_y_pred[over], c='red', alpha=0.5, s=20,
                   label=f'Over ({over.sum()})')
        ax.scatter(y_true_gpclf[under], gpclf_y_pred[under], c='blue', alpha=0.5, s=20,
                   label=f'Under ({under.sum()})')
        ax.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
        ax.set_xlabel('True fraction')
        ax.set_ylabel('Predicted fraction')
        ...
        ax.axvline(np.median(savings), color='darkgreen', ls='-', lw=2,
                   label=f'Median: {np.median(savings):.3f}')
        ax.axvline(np.mean(savings), color='lime', ls='--', lw=2,
        ...
        def _tpr_fpr(y_pred):
            tpr = float(np.mean([actual_recall_at_fraction(y_test_array[:, s], y_pred[s])
                                 for s in range(n_test)]))
        ...
        for mk in model_keys:
            ...
            for approach, param_values, is_clf in [
                ...
                ls = '-' if is_clf else '--'
                ax.plot(fpr_list, tpr_list, color=base_color, ls=ls, lw=1.5,
                ...
                roc_summary[(mk, approach)] = {
                    'auc': auc_val, 'mean_fpr': fpr_opt, 'mean_tpr': tpr_opt,
                }
        ...
        for mk, mu in deterministic_preds.items():
            ...
            tpr, fpr = _tpr_fpr(y_pred)
            ax.scatter(fpr, tpr, s=80, marker='D', label=f'{mk} \u2014 Direct '
                       f'(TPR={tpr:.3f}, FPR={fpr:.3f})')
            roc_summary[(mk, 'Direct')] = {
                'auc': float('nan'), 'mean_fpr': fpr, 'mean_tpr': tpr,
            }

        tpr_gpclf = float(np.mean(gpclf_actual_recalls))
        fpr_gpclf = float(np.mean(gpclf_y_pred))
        ax.scatter(fpr_gpclf, tpr_gpclf, s=80, marker='s',
                   label=f'GPCLFThreshold \u2014 CLF '
                   f'(TPR={tpr_gpclf:.3f}, FPR={fpr_gpclf:.3f})')
        roc_summary[('GPCLFThreshold', 'CLF')] = {
            'auc': float('nan'), 'mean_fpr': fpr_gpclf, 'mean_tpr': tpr_gpclf,
        }
        ...
        for mk_direct in ['RF', 'XGBoost']:
            ...
            roc_summary[(mk_direct, 'Direct+ISO')] = {
                'auc': float('nan'), 'mean_fpr': fpr_iso, 'mean_tpr': tpr_iso,
            }
            ax.scatter(fpr_iso, tpr_iso, s=80, marker='^',
                       label=f'{mk_direct} \u2014 Direct+ISO '
                       f'(TPR={tpr_iso:.3f}, FPR={fpr_iso:.3f})')
        ...
        for frac_name, model_cls, frac_params in [
            ...
            roc_summary[(frac_name, 'Direct')] = {
                'auc': float('nan'), 'mean_fpr': fpr_frac, 'mean_tpr': tpr_frac,
            }
            ax.scatter(fpr_frac, tpr_frac, s=80, marker='v',
                       label=f'{frac_name} \u2014 Direct '
                       f'(TPR={tpr_frac:.3f}, FPR={fpr_frac:.3f})')

        # GP-DirectXGB (stacked meta-model)
        tpr_gpmeta, fpr_gpmeta = _tpr_fpr(gp_meta_preds)
        roc_summary[('GP-DirectXGB', 'Direct')] = {
            'auc': float('nan'), 'mean_fpr': fpr_gpmeta, 'mean_tpr': tpr_gpmeta,
        }
        ax.scatter(fpr_gpmeta, tpr_gpmeta, s=80, marker='*',
                   label=f'GP-DirectXGB \u2014 Direct '
                   f'(TPR={tpr_gpmeta:.3f}, FPR={fpr_gpmeta:.3f})')
...
best_per_tau = perf_df.loc[
    perf_df.groupby('tau')['mean_actual_recall'].idxmax()
...
for (mk, approach), g in perf_df.groupby(['model', 'approach']):
    g = g.sort_values('tau')
    ax.plot(g['tau'], g['mean_actual_recall'], 'o-', lw=1.5,
            label=f'{mk} \u2014 {approach}')
ax.plot([0.82, 1.02], [0.82, 1.02], 'k--', alpha=0.3, label='y = x (perfect)')
ax.axvline(opt_tau, color='red', ls=':', lw=1, alpha=0.7,
...
tau_colors = {t: plt.cm.viridis(i / max(len(TARGET_TAUS) - 1, 1))
              for i, t in enumerate(sorted(TARGET_TAUS))}
...
for _, row in perf_df.iterrows():
    ...
    if label not in seen_labels:
        ...
        ax.scatter(row['loss'], row['mean_actual_recall'],
                   c=[c], marker=m, s=60, alpha=0.8, label=label)
    else:
        ax.scatter(row['loss'], row['mean_actual_recall'],
                   c=[c], marker=m, s=60, alpha=0.8)
    ax.annotate(str(row['model'])[:10], (row['loss'], row['mean_actual_recall']),
                fontsize=5, alpha=0.7)
ax.set_xlabel('Best loss (asymmetric, 3:1)', fontsize=12)
ax.set_ylabel('Mean actual recall', fontsize=12)
ax.set_title(f'Tradeoff: loss vs actual recall across \u03c4 values', fontsize=13, fontweight='bold')
