...
def compute_cross_hit_stats(results: List['DatasetResult']) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    class_stats = per_dataset_df.groupby('class').agg({
    ...
    class_stats.columns = [
        'class',
        'reads_simulated_mean', 'reads_simulated_median', 'reads_simulated_std',
        'reads_simulated_min', 'reads_simulated_max', 'n_simulations',
        'cross_hit_count_mean', 'cross_hit_count_median', 'cross_hit_count_std',
        'cross_hit_count_min', 'cross_hit_count_max',
        'cross_hit_reads_mapped_mean', 'cross_hit_reads_mapped_median', 'cross_hit_reads_mapped_std',
        'cross_hit_reads_mapped_min', 'cross_hit_reads_mapped_max',
        'ratio_mean', 'ratio_median', 'ratio_std', 'ratio_min', 'ratio_max'
    ]
...
def compute_spurious_hit_stats(results: List['DatasetResult']) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    class_stats.columns = [
        ...
        'reads_simulated_mean', 'reads_simulated_median', 'reads_simulated_std',
        'reads_simulated_min', 'reads_simulated_max', 'n_simulations',
        'spurious_hit_count_mean', 'spurious_hit_count_median', 'spurious_hit_count_std',
        'spurious_hit_count_min', 'spurious_hit_count_max',
        'spurious_hit_reads_mapped_mean', 'spurious_hit_reads_mapped_median', 'spurious_hit_reads_mapped_std', 
        'spurious_hit_reads_mapped_min', 'spurious_hit_reads_mapped_max',
        'ratio_mean', 'ratio_median', 'ratio_std', 'ratio_min', 'ratio_max'
    ]
...
def compute_purity(
    ...
    raw_best_match = len(m_stats_matrix[(m_stats_matrix[is_trash_column] == False) & (m_stats_matrix['best_match_is_best'] == True)]) / total
    cov_filtered = len(m_stats_matrix[(m_stats_matrix[is_trash_column] == False) & (m_stats_matrix['coverage'] > 0)]) / total
    
    return raw, raw_best_match, cov_filtered


def compute_mstats_precision(
    m_stats_matrix: pd.DataFrame,
    ...
    unique_matches = unique_matches[(unique_matches['is_trash'] == False) & (unique_matches['best_match_is_best'] == True)]
...
def compute_recall(
    ...
    logger.debug(f"Unique taxids in input: {unique_taxids}, Unique matches in clean m_stats: {len(clean_unique)}, Recall raw: {recall_raw}, Recall coverage filtered: {recall_cov}")
