...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract structured analysis data from simulation outputs")
    parser.add_argument('--study-output', type=str, required=True,
                        help='Path to study output directory')
    parser.add_argument('--ncbi-db', type=str, required=True,
                        help='Path to NCBI taxonomy database (taxa.db)')
    parser.add_argument('--output-dir', type=str, required=True,
                        help='Directory to save analysis outputs')
    parser.add_argument('--cross-hit-threshold', type=float, default=0.3,
                        help='Cross-hit threshold (default: 0.3)')
    parser.add_argument('--min-taxonomic-score', type=float, default=0.7,
                        help='Minimum taxonomic score (default: 0.7)')
    parser.add_argument('--explanatory', action='store_true',
                        help='Run explanatory analysis: mixed-effects model on TP read counts')
    return parser.parse_args()


def discover_datasets(study_output_filepath: str) -> List[str]:
    folders = sorted(
        f for f in os.listdir(study_output_filepath)
        if os.path.isdir(os.path.join(study_output_filepath, f))
    )
    valid = []
    for ds in folders:
...
def load_classifier_map(study_output_filepath: str, dataset: str) -> pd.DataFrame:
    cls_path = os.path.join(
        study_output_filepath, dataset, "classification",
        f"{dataset}_merged_classification.tsv")
...
def process_dataset(
    ...
    m_stats = get_m_stats_matrix(
        dataset, study_output_filepath, ncbi_wrapper, overlap_manager,
...
def collect_recall_data(
    ...
    if not m_stats.empty and 'best_match_is_best' in m_stats.columns:
        tp_taxids = set(
            m_stats.loc[m_stats['best_match_is_best'] == True, 'best_match_taxid']
            .dropna().unique()
        )
...
def fit_tp_relindex_mixed_model(data: pd.DataFrame) -> Optional[object]:
    from statsmodels.regression.mixed_linear_model import MixedLM
...
def fit_recall_gee(data: pd.DataFrame, formula: str) -> Optional[object]:
    ...
    try:
        model = GEE.from_formula(
            formula, groups='data_set', data=df,
...
def save_recall_summary(result, label: str, output_dir: str):
    ...
    pvals = 2 * (1 - scipy_stats.norm.cdf(np.abs(tvals)))
    fe = pd.DataFrame({
        'coef': coef, 'se': se, 'z': tvals, 'p_value': pvals,
    })
    fe['ci_lower'] = fe['coef'] - 1.96 * fe['se']
    fe['ci_upper'] = fe['coef'] + 1.96 * fe['se']
...
def save_model_summary(result, target_name: str, output_dir: str):
    ...
    try:
        ...
        pd.DataFrame(rows).to_csv(
            os.path.join(output_dir, f'model_{target_name}_random_effects.tsv'),
            sep='\t', index=False)
    except Exception:
        pass
    ...
    try:
        ...
        if vc:
            pd.DataFrame([{'component': k, 'variance': v} for k, v in vc.items()]).to_csv(
                os.path.join(output_dir, f'model_{target_name}_variance_components.tsv'),
                sep='\t', index=False)
    except Exception:
        pass
...
def plot_tp_scatters(data: pd.DataFrame, output_dir: str):
    cols = [c for c in ['numreads', 'total_uniq_reads', 'reads_simulated', 'mutation_rate', 'coverage', 'taxid_relindex']
...
def plot_tp_cross_spurious(df: pd.DataFrame, output_dir: str):
    ...
    plot_df = df.melt(
        ...
        var_name='hit_type', value_name='count')
...
def plot_recall_comparison(classifier_df: pd.DataFrame, per_dataset_df: pd.DataFrame, output_dir: str):
    ...
    sns.boxplot(data=plot_df, x='classifier', y='recall', ax=ax,
...
def plot_global_precision_histogram(per_dataset_df: pd.DataFrame, output_dir: str):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.histplot(df['precision_approximate'], kde=True, bins=30,
                 color='seagreen', alpha=0.5, label='Approximate (incl. CrossHits)', ax=ax)
    sns.histplot(df['precision_strict'], kde=True, bins=30,
                 color='steelblue', alpha=0.5, label='Strict (Best Match only)', ax=ax)
    ax.axvline(df['precision_approximate'].median(), color='seagreen',
               linestyle='--', linewidth=1.5)
    ax.axvline(df['precision_strict'].median(), color='steelblue',
               linestyle='--', linewidth=1.5)
...
def plot_precision_per_classifier(classifier_hit_df: pd.DataFrame, per_dataset_df: pd.DataFrame, output_dir: str):
    if classifier_hit_df.empty:
        return
    per_ds_clf = classifier_hit_df.groupby(['data_set', 'classifier']).agg(
    ...
    plot_df = combined[combined['classifier'].isin(order)]
    palette = ['#e74c3c' if c == 'overall' else 'lightblue' for c in order]
    sns.boxplot(data=plot_df, x='classifier', y='precision', ax=ax,
                order=order, palette=palette)
    ax.set_xlabel('Classifier', fontsize=12)
...
def main():
    ...
    for ds in datasets:
        try:
            result = process_dataset(
                ds, study_output, ncbi_wrapper,
                args.cross_hit_threshold, args.min_taxonomic_score,
    ...
    if not classifier_df.empty:
        ...
        agg_classifier = classifier_df.groupby('classifier').agg(
