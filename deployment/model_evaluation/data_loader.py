...
def output_parse(
    study_output_filepath: str, ncbi_wrapper: NCBITaxonomistWrapper, input_taxids: list[int]
) -> pd.DataFrame:
    """
...
def establish_taxids_to_use(
    ...
    taxids_to_use = (
        taxids_to_use.dropna(subset=[tax_level_to_use])
        .drop_duplicates(subset=[tax_level_to_use])
        .reset_index(drop=True)
    )

    unclassified_row = pd.DataFrame(
        {"taxid": [0], "order": ["unclassified"], "family": ["unclassified"], "genus": ["unclassified"]}
    )
...
def expand_input_data(
    all_input_data: pd.DataFrame, ncbi_wrapper: NCBITaxonomistWrapper, taxid_plan: pd.DataFrame
) -> pd.DataFrame:
    """
...
def get_dataset_folders(study_output_filepath: str, max_training: int | None = None) -> list[str]:
    """
...
def split_train_test_folders(folders: list[str], proportion_train: float) -> tuple[list[str], list[str]]:
    """
...
class DataLoader:
    ...
    def __init__(self, config):
        ...
        """
        self.config = config
        self.ncbi_wrapper: NCBITaxonomistWrapper | None = None
        self.all_input_data: pd.DataFrame | None = None
        self.input_tax_df: pd.DataFrame | None = None
        self.taxids_to_use: pd.DataFrame | None = None
        self.taxid_plan: pd.DataFrame | None = None
    ...
    def _load_datasets(self):
        ...
        self.folders = get_dataset_folders(self.config.study_output_filepath, self.config.max_training)
