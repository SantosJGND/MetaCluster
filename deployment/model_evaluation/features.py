...
class RecallFeatureTransformer(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, tax_level: str = "order", data_set_divide: int = 5, sort_strategy: SORT_STRATEGIES = "reads"):
        self.tax_level = tax_level
        self.data_set_divide = data_set_divide
        self.sort_strategy = sort_strategy
        self.reference_taxa_: list[str] | None = None
    ...
    def fit(
        ...
        logger.debug(
            "RecallFeatureTransformer fitted: %d stat cols + %d composition cols (%d total features)",
            len(stat_cols),
            len(comp_cols),
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        best_match_indices = m_stats.index[m_stats["best_match_is_best"] == True].tolist()
        last_best_match_index = best_match_indices[-1] + 1 if best_match_indices else 0
        last_best_match_relindex = last_best_match_index / len(m_stats) if len(m_stats) > 0 else 0.0
    ...
    def target_columns_(self) -> list[str]:
        ...
        return ["last_best_match_relindex"] + [f"index_recall_{i}" for i in range(1, self.data_set_divide + 1)]
    ...
    def get_feature_names_out(self) -> list[str]:
        ...
        if self._feature_names is None:
            raise RuntimeError("get_feature_names_out() called before fit(). Call fit() first.")
