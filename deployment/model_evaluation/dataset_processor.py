...
class DatasetProcessor:
    ...
    def _compute_baseline_metrics(
        ...
        try:
            spurious_comp = get_spurious_composition(
                result.input_df, m_stats, self.taxids_to_use,
                tax_level=self.config.tax_level
            )
        ...
        try:
            ...
            input_composition = get_subset_composition(result.input_df, self.taxids_to_use, tax_level=self.config.tax_level).set_index('tax_level').T
    ...
    def _apply_fixed_filter(
        ...
        overlap_manager = OverlapManager(
            os.path.join(self.config.study_output_filepath, f"{data_set_name}", "clustering"),
    ...
    def _apply_recall_filter(
        ...
        new_m_stats = new_m_stats.head(metrics_dict.get('keep_index', len(new_m_stats)))  # Ensure we only take the top keep_index after filtering
        new_m_stats = new_m_stats[new_m_stats['coverage'] > 0]  # Filter out zero coverage leaves for recall calculation

        logger.debug(f"######### RECALL FILTER M-STATS {data_set_name} #########")
        logger.debug(f"{new_m_stats.shape}, {filtered_om.original_m_stats_matrix.shape}, {self.config.target_recall}, {metrics_dict.get('target_percentile', None)}, {metrics_dict.get('keep_index', None)}")
    ...
    def _apply_crosshit_cleanup(
        ...
        cross_hit_f1 = safe_divide(2 * cross_hit_precision * cross_hit_recall, 
                                   cross_hit_precision + cross_hit_recall)
