...
def analyze_cross_hit_distribution(results: list, output_dir: str, top_n: int = 15):
    """
    ...
    if top_classes:
        ...
        for metric, ylabel in [
            ...
            sns.boxplot(data=plot_df, x="class", y=metric, order=top_classes, ax=ax, color="steelblue")
        ...
        sns.scatterplot(data=plot_df, x="reads_simulated", y="cross_hit_count", ax=ax, alpha=0.6)
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        sns.scatterplot(data=plot_df, x="reads_simulated", y="cross_hit_reads_mapped", ax=ax, alpha=0.6)
        ax.set_xlabel("Reads Simulated", fontsize=12)
...
def analyze_spurious_hit_distribution(results: list, output_dir: str, top_n: int = 15):
    """
    ...
    if top_classes:
        ...
        for metric, ylabel in [
            ...
            fig, ax = plt.subplots(figsize=(12, 6))
            sns.boxplot(data=plot_df, x="class", y=metric, order=top_classes, ax=ax, color="salmon")
            ax.set_xlabel("Class", fontsize=12)
            ax.set_ylabel(ylabel, fontsize=12)
            ax.tick_params(axis="x", rotation=45)
            ax.grid(axis="y", alpha=0.3)
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        sns.scatterplot(data=plot_df, x="reads_simulated", y="spurious_hit_count", ax=ax, alpha=0.6)
        ax.set_xlabel("Reads Simulated", fontsize=12)
        ...
        fig, ax = plt.subplots(figsize=(10, 8))
        sns.scatterplot(data=plot_df, x="reads_simulated", y="spurious_hit_reads_mapped", ax=ax, alpha=0.6)
        ax.set_xlabel("Reads Simulated", fontsize=12)
...
def get_args():
    ...
    parser.add_argument(
        "--target_recall", type=float, default=1.0, help="Target recall threshold for filtering (default: 1.0)"
    )
    parser.add_argument(
        "--holdout_proportion", type=float, default=0.3, help="Proportion of data to hold out for testing"
    )
    ...
    parser.add_argument(
        "--max_taxids_fixed_filter", type=int, default=15, help="Maximum number of taxids for fixed filter"
    )
    parser.add_argument(
        "--recall_model_interface",
        type=str,
        default="xgb",
        help="Model interface: 'xgb' (XGBoost multi-output), 'morf' (RF multi-output), 'moxgb_optimized', 'morf_optimized', 'monn_optimized', 'direct' (RF cutoff classifier), 'direct_xgb' (DirectXGB regressor), 'gp_clf' (GP+CLF)",
    )
    parser.add_argument(
        "--composition_model_interface",
        type=str,
        default="xgb",
        help="Composition model: 'xgb' (default, XGBoost), 'xgb_optimized' (XGBoost+Optuna), 'rf' (RandomForest), 'gb' (GradientBoosting), 'lr' (LogisticRegression, stats-only)",
    )
    parser.add_argument(
        "--cutoff_confidence",
        type=float,
        default=None,
        help="Confidence for probability-guided cutoff (0-1, default: point prediction)",
    )
    parser.add_argument(
        "--recall_sort_strategy",
        type=str,
        default="reads",
        help="Sort strategy for recall: 'reads', 'taxid_roundrobin', 'rarity_boost', 'tax_level_stratified'",
    )
    parser.add_argument(
        "--cross_hit_threshold", type=float, default=0.95, help="Cross-hit probability threshold for filtering"
    )
    parser.add_argument(
        "--enable-cross-hit",
    ...
    parser.add_argument(
        "--generate-report", action="store_true", default=True, help="Generate HTML report (default: enabled)"
    )
...
def main(args):
    ...
    if config.use_mlflow:
        try:
            mlflow_tracker = MLflowTracker(experiment_name="metagenomics-evaluation", tracking_uri=config.mlflow_uri)
    ...
    if config.use_cache is False:
        trainer._stats_matrices.to_csv(
            str(config.analysis_output_filepath / "training_stats_matrices.tsv"), sep="\t", index=False
        )
    ...
    if mlflow_tracker:
        final_metrics = {
            "test_precision_mean": results.test_results["precision_clade_post"].mean()
            if not results.test_results.empty
            else 0,
            "test_precision_std": results.test_results["precision_clade_post"].std()
            if not results.test_results.empty
            else 0,
