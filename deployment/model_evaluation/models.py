...
class ModelTrainer:
    ...
    def __init__(
        ...
        self.crosshit_modeller: CrossHitModeller | None = None
    ...
    def run_data_retrieval(self, training_folders: list) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(
            f"Retrieved training data: {len(training_results_df)} results, {len(prediction_results_df)} predictions"
        )
    ...
    def _get_m_stats_matrix(
        ...
        return get_m_stats_matrix(
            data_set_name, self.config.study_output_filepath, self.ncbi, overlap_manager, filter_no_leaf=filter_no_leaf
        )
