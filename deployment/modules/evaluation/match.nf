...
process MatchCladeReportWithReferenceSequences {
    ...
    clade_report_path = "${clade_report}"
    if os.path.exists(clade_report_path) and os.path.getsize(clade_report_path) > 0:
        clade_report = pd.read_csv(clade_report_path, sep="\\t", header=None,  names=["clade", "nuniq", "freq", "min_pair_dist", "nfiles", "files"])
        clade_report['clade']
    ...
    clade_report_with_references = clade_report_with_references[['description', 'taxid', 'assembly_accession', \
            'coverage', 'clade', 'nuniq', 'freq', 'classifiers']]
