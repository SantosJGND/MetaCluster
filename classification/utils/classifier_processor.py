...
from metagenomics_utils.dataframe_utils import setup_logger

logger = setup_logger(__name__)


def taxid_to_description(taxid: int) -> str | None:
    """
...
def protein_accession_to_taxid(accession: str) -> int | None:
    """
...
class ClassifierOutputProcessor(ABC):
    ...
    def __init__(self, output_path: str) -> None:
        ...
        self.software_name = ""
        self.final_report: pd.DataFrame = pd.DataFrame(columns=["description", "taxID", "software_name"])
...
class KrakenUniqOutputProcessor(ClassifierOutputProcessor):
    ...
    def __init__(self, output_path: str, min_uniq_reads: int = 10) -> None:
    ...
    def from_file(cls, output_path: str) -> KrakenUniqOutputProcessor:
        ...
        processor = cls(output_path)
        try:
            report = pd.read_csv(output_path, sep="\t")
            report.columns = ["status", "sequence_id", "taxID", "query_length", "map_length"]
            processor.report = report[report["status"] == "C"]
        except (pd.errors.EmptyDataError, FileNotFoundError):
            processor.report = pd.DataFrame(columns=["status", "sequence_id", "taxID", "query_length", "map_length"])
        return processor
...
class DiamondOutputProcessor(ClassifierOutputProcessor):
    ...
    def __init__(self, output_path: str, min_uniq_reads: int = 5) -> None:
        super().__init__(output_path)
        self.min_uniq_reads = min_uniq_reads
...
class CentrifugeOutputProcessor(ClassifierOutputProcessor):
    ...
    def __init__(self, output_path: str, nuniq_threshold: int = 5) -> None:
        super().__init__(output_path)
    ...
    def from_file(cls, output_path: str) -> CentrifugeOutputProcessor:
        ...
        processor = cls(output_path)
        try:
            processor.report = pd.read_csv(output_path, sep="\t")
        except (pd.errors.EmptyDataError, FileNotFoundError):
            processor.report = pd.DataFrame(columns=["name", "taxID", "numUniqueReads"])
        return processor
    ...
    def process(self) -> CentrifugeOutputProcessor:
        ...
        self.final_report = self.report[["name", "taxID", "numUniqueReads"]].rename(
            columns={"name": "description", "numUniqueReads": "uniq_reads"}
        )
        return self
...
class KrakenOutputProcessor(ClassifierOutputProcessor):
    ...
    def from_file(cls, output_path: str) -> KrakenOutputProcessor:
        ...
        processor = cls(output_path)
        try:
            kraken_report = pd.read_csv(output_path, sep="\t", header=None)
        ...
        processor.nodes_dict = {node[0]: node for node in nodes}
        processor.edges = edges
        processor.report = kraken_report
    ...
    def process(self) -> KrakenOutputProcessor:
        ...
        self.final_report = self.final_report[["description", "taxID", "Nreads"]].rename(
            columns={"Nreads": "uniq_reads"}
        )
        return self

    @staticmethod
    def kraken_report_to_tree(report: pd.DataFrame) -> tuple[list[KrakenNode], list[KrakenEdge]]:
    ...
    @staticmethod
    def get_leaves(nodes: list[KrakenNode], edges: list[KrakenEdge]) -> list[int]:
        """Get leaf nodes from tree."""
        G = nx.DiGraph()
        G.add_edges_from(edges)
        return [node[0] for node in nodes if G.out_degree(node[0]) == 0]

    def get_simplified_leaves(self, nodes: list[KrakenNode], edges: list[KrakenEdge]) -> dict[int, list[int]]:
    ...
    @staticmethod
    def summarize_leaves(leaves_simple: dict[KrakenNode, list[KrakenNode]]) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            summary, columns=["taxID", "description", "prefix_spaces", "perc_reads", "Nreads", "rank_code"]
        )
