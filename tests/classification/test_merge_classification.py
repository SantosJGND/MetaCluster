...
class TestMergeClassificationResults:
    ...
    def kraken2_file(self, temp_dir):
        ...
        create_classifier_output(path, {"name": ["virus_a", "virus_b"], "taxID": [123, 456], "uniq_reads": [100, 50]})
        return path

    @pytest.fixture
    def centrifuge_file(self, temp_dir):
        path = temp_dir / "centrifuge.tsv"
        create_classifier_output(path, {"name": ["virus_a", "virus_c"], "taxID": [123, 789], "uniq_reads": [80, 30]})
        return path

    @pytest.fixture
    def diamond_file(self, temp_dir):
        path = temp_dir / "diamond.tsv"
        create_classifier_output(path, {"name": ["virus_b", "virus_d"], "taxID": [456, 999], "uniq_reads": [25, 15]})
        return path
    ...
    def test_merge_kraken2_and_centrifuge(self, temp_dir, kraken2_file, centrifuge_file):
        ...
        merged = pd.merge(
            kraken2_df, centrifuge_df, on=["description", "taxid"], how="outer", suffixes=("_kraken2", "_centrifuge")
        )
    ...
    def test_merge_with_diamond(self, temp_dir, kraken2_file, centrifuge_file, diamond_file):
        ...
        merged = pd.merge(merged, diamond_df, on=["description", "taxid"], how="outer", suffixes=("", "_diamond"))
...
class TestMatchCladeReportWithReferenceSequences:
    ...
    def coverage_report_file(self, temp_dir):
        ...
        df = pd.DataFrame({"file": ["assembly1", "assembly2"], "coverage": [10.5, 8.2]})
        df.to_csv(path, sep="\t", index=False)
        return path
    ...
    def merge_classification_file(self, temp_dir):
        ...
        df = pd.DataFrame({"taxid": [562, 28901], "classifier": ["kraken2", "kraken2"]})
        df.to_csv(path, sep="\t", index=False)
        return path
    ...
    def test_missing_clade_report_sets_unmapped(
        self, temp_dir, matched_assemblies_file, coverage_report_file, merge_classification_file
    ):
        ...
        if not os.path.exists(clade_report_path_str) or os.path.getsize(clade_report_path_str) == 0:
            ...
            clade_report = pd.read_csv(
                clade_report_path_str,
                sep="\t",
                header=None,
    ...
    def test_valid_clade_report_maps_correctly(
        self,
        temp_dir,
        matched_assemblies_file,
        coverage_report_file,
        merge_classification_file,
        valid_clade_report_file,
    ):
    ...
    def test_clade_report_with_references_output_structure(
        ...
        result["nuniq"] = 0
        result["freq"] = 0
        result["classifier"] = result["taxid"].map(dict(zip(merge_results["taxid"], merge_results["classifier"])))
        result["coverage"] = result["assembly_accession"].map(
            lambda x: (
                coverage_report[coverage_report["file"].str.contains(x, na=False)]["coverage"].values[0]
                if not coverage_report[coverage_report["file"].str.contains(x, na=False)].empty
                else 0
            )
        )
