...
def test_classify_no_cluster_analysis_flag(reads_dir, temp_output_dir, project_root, cleanup_nextflow):
    ...
    analysis_dirs = list(output_dir.glob("no_cluster_test*"))
    assert len(analysis_dirs) > 0, "No analysis output directory found"

    analysis_dir = analysis_dirs[0]
    clustering_dir = analysis_dir / "clustering"

    if clustering_dir.exists():
        distance_matrix_files = list(clustering_dir.glob("distance_matrix*"))
        assert len(distance_matrix_files) > 0, "distance_matrix.tsv should be produced with --no-cluster-analysis"

        clade_report_files = list(clustering_dir.glob("clade_report*"))
        assert len(clade_report_files) == 0, "clade_report.tsv should NOT be produced with --no-cluster-analysis"
