...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Simulate reads files using wgsim from reference genomes provided in a table."
    )
    parser.add_argument("--input_table", type=str, required=True, help="Path to the classification output file.")
    parser.add_argument("--output_dir", type=str, required=True, help="Path to the output directory.")

    parser.add_argument("--prefix", type=str, default="fastq", help="Prefix for the output files.")
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        print(
            f"Simulating {nreads} reads for genome {genome_id} with mutation rate {mut_rate if mut_rate else 'default'}..."
        )
